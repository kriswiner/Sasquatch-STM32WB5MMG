 ...
 // Check RTC time, RTC kept alive in STANDBT mode
  ...
  if(SerialDebug)  {
    ...
    // add system info to the flash page
      ...
      flashPage[255] = 0x73;  // add a marker to signify page has been programmed in lieu of SRAM data save
 
      SFLASH.begin();
      SFLASH.program(page_number * 256, flashPage, sizeof(flashPage));  // write next 256-byte page
      while (SFLASH.busy()) { }
      SFLASH.end();
      if(SerialDebug) {
