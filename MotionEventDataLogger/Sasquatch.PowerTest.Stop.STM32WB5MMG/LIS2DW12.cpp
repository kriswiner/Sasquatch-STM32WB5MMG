...
void LIS2DW12::init(uint8_t fs, uint8_t odr, uint8_t mode, uint8_t lpMode, uint8_t bw, bool lowNoise, bool stMode)
{
   ...
   _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_CTRL3, 0x10); // push pull, active high, latched   
   ...
   _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_CTRL5_INT2_PAD_CTRL, 0x40); // only need wakeup interrupt for package monitoring application
   ...
   // set stationary bit 4
              _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_WAKE_UP_DUR, 0x20 | 0x00 );  // set inactivity delay before sleep to 10s at 1.6 Hz odr
   if(stMode) _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_WAKE_UP_DUR, 0x20 | 0x10 );  // set inactivity delay before sleep to 10s at 1.6 Hz odr

   // pulse interrupt (bit 7), enable interrupt (bit 5), route all INT2 signals to INT1 (bit 6)
   _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_CTRL_REG7, 0x80 | 0x40 | 0x20);    
}


void LIS2DW12::activateNoMotionInterrupt()
{
  _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_CTRL4_INT1_PAD_CTRL, 0x20); // enable wake-on-motion interrupt on INT1
}


void LIS2DW12::deactivateNoMotionInterrupt()
{
  _i2c_bus->writeByte(LIS2DW12_ADDRESS, LIS2DW12_CTRL4_INT1_PAD_CTRL, 0x00); // disable wake-on-motion interrupt on INT1
}
...
float LIS2DW12::HalftoFloat(uint16_t n)
{
