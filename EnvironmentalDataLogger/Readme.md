...
The QSPI log uses fixed 256-byte pages with a 32-byte page header and four 56-byte records per page. Each record is a compact, timestamped environmental snapshot containing only: temperature, humidity, pressure, gas readings, RGB/IR/lux, battery voltage, AEM charging telemetry, acceleration, AEM policy state, and one quality byte. A helper sketch reads the flash after an experiment concludes, translates the logged data into propoer units, if needed, and outputs the data in CSV format for easy import into a spreadsheet for display and analysis.
