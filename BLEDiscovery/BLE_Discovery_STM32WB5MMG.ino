...
const bool LOG_FLASH_WRITES = false;            // Set true to print each programmed flash page.
...
const bool VERBOSE_OUTPUT = false;              // Set true for the full decoded packet output.
const bool PASSIVE_SCAN = false;                // Set false for active scanning and scan-response data.
const bool SCAN_WITH_DUPLICATES = true;         // Needed for useful RSSI smoothing and seen counts.
const bool USE_STOP_BETWEEN_EVENTS = false;     // Leave false until STOP mode is reliable in this core.
const bool BLINK_ON_BLE_REPORT = true;          // Blue report pulse is useful during early field testing.
const bool RGB_LED_ACTIVE_LOW = true;           // Sasquatch RGB LED channels turn on when driven LOW.
...
void idleBetweenBleEvents();
void sampleEnvironment();
void setupStatusLed();
void ledOff(uint8_t pin);
void ledBlink(uint8_t pin, unsigned long durationMs);
...
void setup()
{
    ...
    BLE.scan(SCAN_WITH_DUPLICATES ? BLE_SCAN_MODE_NONE : BLE_SCAN_MODE_WITHOUT_DUPLICATES);
...
void loop()
{
    ...
    if (BLE.report(address, rssi, event, data)) {
        ...
        if (BLINK_ON_BLE_REPORT) {
            ledBlink(PIN_LED_BLUE, REPORT_LED_PULSE_MS);
        }
    ...
    if (summaryDue) {
        ...
        summaryDue = false;
        interrupts();
        ledBlink(lowBattery ? PIN_LED_RED : PIN_LED_GREEN,
    ...
    // Future battery-life hook: when STOP mode is fixed in the core, this can
    // idle the MCU between BLE stack events and TimerMillis wakeups.
    if (!handledReport && !summaryDue) {
        idleBetweenBleEvents();
    }
}
...
void ledOff(uint8_t pin)
{
    digitalWrite(pin, RGB_LED_ACTIVE_LOW ? HIGH : LOW);
}

void ledBlink(uint8_t pin, unsigned long durationMs)
{
    digitalWrite(pin, RGB_LED_ACTIVE_LOW ? LOW : HIGH);
    delay(durationMs);
    ledOff(pin);
}
...
void printStartupInfo(const char scannerAddress[])
{
    if (!SERIAL_DEBUG || !PRINT_STARTUP_INFO) {
        return;
    }
    ...
    Serial.println(SCAN_WITH_DUPLICATES ? "yes" : "no");
    Serial.print("Stop between events: ");
    Serial.println(USE_STOP_BETWEEN_EVENTS ? "yes" : "no");
...
bool commitLogPage()
{
    ...
    if (SERIAL_DEBUG && Serial && LOG_FLASH_WRITES) {
        Serial.print("Log page ");
        Serial.print(nextLogPage);
        Serial.print(" type ");
        Serial.println(logPage[3]);
    }
