...
const bool PRINT_RAW_ROWS = false;
...
struct RollupAnalysis {
    ...
    uint16_t uniqueDevices;
    uint16_t uniqueApple;
    uint16_t uniqueSt;
    uint16_t uniqueGoogle;
    uint16_t uniqueOtherKnown;
    uint16_t uniqueUnknown;
    uint16_t uniqueNamed;
    uint16_t uniquePublicAddress;
    uint16_t uniqueRandomStatic;
    uint16_t uniqueStrong;
    uint16_t uniqueMedium;
    uint16_t uniqueWeak;
};
...
uint16_t envCheckpointCount = 0;
uint16_t firstBatteryMv = 0;
uint16_t lastBatteryMv = 0;
uint16_t minBatteryMv = 0xFFFF;
uint16_t maxBatteryMv = 0;
int16_t firstMcuTempCenti = 0;
int16_t lastMcuTempCenti = 0;
int16_t firstLis2dw12TempCenti = 0;
int16_t lastLis2dw12TempCenti = 0;
...
void printSessionPage(uint32_t physicalPage)
{
    ...
    Serial.print(",top_n=");
    Serial.print(flashPage[34]);
    Serial.print(",reset=0x");
    Serial.print(getU32(35), HEX);
    Serial.print(",wakeup=0x");
    Serial.print(getU32(39), HEX);
    Serial.print(",reason=0x");
    Serial.println(getU32(43), HEX);
}
...
void printCheckpointPage(uint32_t physicalPage)
{
    uint16_t batteryMv = getU16(30);
    int16_t mcuTempCenti = getI16(32);
    int16_t lis2dw12TempCenti = getI16(34);

    if (envCheckpointCount == 0) {
        firstBatteryMv = batteryMv;
        firstMcuTempCenti = mcuTempCenti;
        firstLis2dw12TempCenti = lis2dw12TempCenti;
    }

    lastBatteryMv = batteryMv;
    lastMcuTempCenti = mcuTempCenti;
    lastLis2dw12TempCenti = lis2dw12TempCenti;
    if (batteryMv < minBatteryMv) minBatteryMv = batteryMv;
    if (batteryMv > maxBatteryMv) maxBatteryMv = batteryMv;
    envCheckpointCount++;
    ...
    Serial.print("mV,mcu=");
    Serial.print(mcuTempCenti / 100.0f, 2);
    Serial.print("C,lis2dw12=");
    Serial.print(lis2dw12TempCenti / 100.0f, 2);
    Serial.println("C");
}
...
void updateDeviceAnalysis(uint16_t offset, uint32_t batch)
{
    ...
    if (rollupCount > 0) {
        uint16_t rollupIndex = rollupCount - 1;
        RollupAnalysis &rollup = rollups[rollupCount - 1];
        uint16_t currentMfgId = getU16(offset + 14);
        uint16_t currentNameHash = getU16(offset + 20);
        uint8_t currentAddressType = flashPage[offset + 6];
        ...
        if (device.mfgId == 0x004C) {
            ...
            rollup.google++;
        } else if (device.mfgId == 0x0000 || device.mfgId == 0xFFFF) {
            rollup.unknown++;
        } else {
            rollup.otherKnown++;
        }
        ...
        if (device.lastRollupIndex != rollupIndex) {
            device.lastRollupIndex = rollupIndex;
            rollup.uniqueDevices++;
            ...
            if (currentMfgId == 0x004C) {
                rollup.uniqueApple++;
            } else if (currentMfgId == 0x0030) {
                rollup.uniqueSt++;
            } else if (currentMfgId == 0x00E0) {
                rollup.uniqueGoogle++;
            } else if (currentMfgId == 0x0000 || currentMfgId == 0xFFFF) {
                rollup.uniqueUnknown++;
            } else {
                rollup.uniqueOtherKnown++;
            }

            if (currentNameHash != 0) {
                rollup.uniqueNamed++;
            }

            if (currentAddressType == 0) {
                rollup.uniquePublicAddress++;
            } else if (currentAddressType == 2) {
                rollup.uniqueRandomStatic++;
            }
        }
    }
}
...
void printAnalysisSummary()
{
    uint32_t durationSeconds = totalIntervals * (uint32_t)batchIntervalSeconds;
...
{
    Serial.println();
    Serial.println("Environment checkpoints");
    ...
    Serial.println(envCheckpointCount);
    Serial.print("  battery first:     ");
    Serial.print(firstBatteryMv);
    Serial.println(" mV");
    Serial.print("  battery last:      ");
    Serial.print(lastBatteryMv);
    Serial.println(" mV");
    Serial.print("  battery range:     ");
    Serial.print(minBatteryMv);
    Serial.print("-");
    Serial.print(maxBatteryMv);
    Serial.println(" mV");
    Serial.print("  MCU temp first:    ");
    Serial.print(firstMcuTempCenti / 100.0f, 2);
    Serial.println(" C");
    Serial.print("  MCU temp last:     ");
    Serial.print(lastMcuTempCenti / 100.0f, 2);
    Serial.println(" C");
    Serial.print("  LIS2DW12 first:    ");
    Serial.print(firstLis2dw12TempCenti / 100.0f, 2);
    Serial.println(" C");
    Serial.print("  LIS2DW12 last:     ");
    Serial.print(lastLis2dw12TempCenti / 100.0f, 2);
    Serial.println(" C");
}
...
void printRollupSummary()
{
    ...
    for (uint16_t i = 0; i < rollupCount; i++) {
        ...
        Serial.println(rollup.uniqueDevices);
        printRollupValue("unique strong", rollup.uniqueStrong);
        printRollupValue("unique medium", rollup.uniqueMedium);
        printRollupValue("unique weak", rollup.uniqueWeak);
        printRollupValue("unique Apple", rollup.uniqueApple);
        printRollupValue("unique ST", rollup.uniqueSt);
        printRollupValue("unique Google", rollup.uniqueGoogle);
        printRollupValue("unique other", rollup.uniqueOtherKnown);
        printRollupValue("unique unknown", rollup.uniqueUnknown);
        printRollupValue("unique named", rollup.uniqueNamed);
        printRollupValue("unique public", rollup.uniquePublicAddress);
        printRollupValue("unique static", rollup.uniqueRandomStatic);
        ...
        printRollupValue("other obs", rollup.otherKnown);
        printRollupValue("unknown obs", rollup.unknown);
...
void printDuration(uint32_t seconds)
{
    ...
    uint8_t minutes = (seconds / 60UL) % 60;
