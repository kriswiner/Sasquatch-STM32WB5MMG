...
void setup() {
  ...
  if(sensorOK) sensorOK = LIS2DW12.configureFIFO(CONT_TO_FIFO, 0x1F); // Start FIFO history capture.
...
//  RTC.enableAlarm(RTC_MATCH_HHMMSS);  // alarm once a day
  ...
  if(!LIS2DW12.getStatus(&LIS2DW12_status)) {                    // Read status once to clear any stale interrupt.
    if(SerialDebug) Serial.println("LIS2DW12 status clear failed; device is not operational.");
...
void loop() {
 ...
 //   if(1) {                                            // do even if the FIFO threshold is not reached
    if(FIFOstatus & 0x80) {                            // if the FIFO threshold is reached
      ...
      bool accelReadOK = true;                         // Require every FIFO sample before committing.

      for(uint8_t i = 0; i < numFIFOSamples; i++) {    // read the FIFO data
         if(!LIS2DW12.readAccelData(accelCount)) {     // get 14-bit signed accel data
           accelReadOK = false;
           sensorOK = false;
           break;
         }
  ...
  if(SerialDebug)  {
    ...
    Serial.print("VBAT = "); Serial.println(VBAT, 2);  Serial.println(" ");
      ...
      iTemperature = FloattoHalf(Temperature);         // convert float system data to half-float data for efficient storage
   ...
   if(!LIS2DW12.configureFIFO(CONT_TO_FIFO, 0x1F)) sensorOK = false; // Restart FIFO history capture.
   ledBlink(red, 1); // toggle red led when motion detected 
   }
