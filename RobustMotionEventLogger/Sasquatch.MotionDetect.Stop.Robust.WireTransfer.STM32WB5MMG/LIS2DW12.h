...
#define LIS2DW12_ADDRESS  0x19  // if SDO/SA0 is 1 (default since internally pulled up by ~30 kOhm resistor)
// #define LIS2DW12_ADDRESS  0x18  // if SDO/SA0 is pulled to GND
...
class LIS2DW12
{
  ...
  bool init(uint8_t fs, uint8_t odr, uint8_t mode, uint8_t lpMode, uint8_t bw, bool lowNoise, bool stMode);
  bool Compensation(uint8_t fs, uint8_t odr, uint8_t mode, uint8_t lpMode, uint8_t bw, bool lowNoise, float *offset);
  bool reset();
  bool selfTest(float *destination);
  bool readAccelData(int16_t *destination);
  bool readFIFOAccelData(int16_t destination[][3], uint8_t count);
  bool readTempData(int16_t *temperature);
  bool readRawTempData(uint8_t *temperature);
  bool activateWakeOnMotionInterrupt();
  bool deactivateWakeOnMotionInterrupt();
  bool activateFIFOThresholdInterrupt();
  bool deactivateFIFOThresholdInterrupt();
  bool activateSleepChangeInterrupt();
  bool deactivateSleepChangeInterrupt();
  bool getStatus(uint8_t *status);
  bool powerDown();
  bool powerUp(uint8_t odr);
  bool getWakeSource(uint8_t *source);
