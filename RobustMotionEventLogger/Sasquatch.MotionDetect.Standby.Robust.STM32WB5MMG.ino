/*
  Sasquatch low-power motion-event logger.

  The LIS2DW12 watches for motion while the STM32WB5MMG is in STANDBY. A motion
  wake reads the 32-sample accelerometer FIFO, adds event and system metadata,
  writes one 256-byte QSPI page, and returns to STANDBY. SRAM2 normally retains
  the next page number; after lost SRAM2 state, a short binary search resumes at
  the first erased page without erasing earlier records.

  Record format version 2:
    bytes   0-191  32 XYZ samples, three half-floats per sample
    bytes 192-208  version, sample count, page, event/configuration, RTC, system
    bytes 209-252  reserved for future metadata
...
RetainedLogState __SECTION_BBRAM_NOINIT logState;
...
*/
  ...
  if(wakeReason == WAKEUP_PIN_3) {                     // this is the LIS2DW12 interrupt
   ...
   LIS2DW12.deactivateWakeOnMotionInterrupt(); // only want the first  detection of the crossing of the motion threshold
...
/*    SerialBLE.print("Temp = ");
   ...
   LIS2DW12.activateWakeOnMotionInterrupt(); // allow next detection of crossing of the motion threshold
...
/*    SerialBLE.print("Temp = ");
  ...
  bool flashReady = SFLASH.begin();
  if(flashReady) flashReady = SFLASH.identify(mid, did);
  ...
  if(!flashReady) setLogState(logState.page, true);
  if(flashReady) SFLASH.end();
...
// Helper to calculate a CRC over a QSPI flash page to prevent/detect data corruption
