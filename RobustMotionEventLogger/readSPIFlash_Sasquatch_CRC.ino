/*
  ...
  page number (version 2), and CRC before any values are decoded. Newer version-2
  records also print the optional 96-bit STM32WB UID stored in reserved metadata
  bytes. Lines beginning with '#' describe an event; the remaining comma-delimited
  lines are suitable for plotting or spreadsheet analysis.
*/
...
{ 
  ...
  if(!flashReady) {
    ...
    if(flashStarted) SFLASH.end();                              // Release flash power after a failed identification.
    return;
  }
  ...
  for(page_number = 0; page_number < max_page_number; page_number++) {
    ...
    if(recordVersion == RECORD_VERSION_2) {
      ...
      uidPresent = false;                               // Older version-2 records leave the UID bytes erased.
      for(uint8_t i = 0; i < UID_LENGTH; i++) {
        storedUID[i] = flashPage[UID_START_BYTE + i];   // Copy the UID in stored big-endian byte order.
        if(storedUID[i] != 0xFF) uidPresent = true;
      }
    }
    else {
      ...
      uidPresent = false;                               // Version 1 did not store the MCU UID.
    }
    ...
    if(recordVersion == RECORD_VERSION_2) {
      ...
      if(uidPresent) {
        Serial.print(",UID=");                         // Print a fixed 24-digit hexadecimal identifier.
        for(uint8_t i = 0; i < UID_LENGTH; i++) {
          if(storedUID[i] < 0x10) Serial.print("0");
  ...
  // The logger stores the trusted FIFO samples in chronological order:
  // flash sample 0 is the earliest and sampleCount - 1 is the latest.
  for(uint8_t i = 0; i < sampleCount; i++) {
      ...
      Serial.print(i + 1); Serial.print(",");   // Chronological sample number, acceleration in milligs
