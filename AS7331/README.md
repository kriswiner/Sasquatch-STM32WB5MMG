...
This repository contains a robust, ultra-low-power AS7331 UVA/UVB/UVC demonstration for the Tlera Sasquatch STM32WB5MMG board. The sensor is connected to the external `Wire` I2C bus, with its push-pull READY interrupt connected to GPIO 8. Measurements use AS7331 command mode with gain 8 and integration-time setting 6. Interrupt and timer callbacks only set flags and wake the MCU; the main loop services the resulting state machines and otherwise enters `STM32WB.stop()`. Measured STOP current was approximately 5 µA.
